...
const { OfficialModules, parseCentralToml } = require('../modules/official-modules');
...
class Installer {
  ...
  async _installAndConfigure(
    ...
    const configTask = {
      ...
      task: async (message) => {
        await this._removeLegacyModuleConfigs(paths.bmadDir);
  ...
  async _prepareUpdateState(paths, config, existingInstall, officialModules) {
    ...
    if (!config.coreConfig || Object.keys(config.coreConfig).length === 0) {
      const existingCoreConfig = await this._readInstalledCoreConfig(paths.bmadDir);
      if (Object.keys(existingCoreConfig).length > 0) {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            // Skip config.yaml files - leftovers from the retired per-module
            // yaml regime. Never user-authored; _removeLegacyModuleConfigs
            // deletes them later in this same run, so they must not be picked
            // up here and restored afterwards as "custom" or "modified".
  ...
  /**
   * Remove the per-module `_bmad/<module>/config.yaml` files earlier installers
   * generated. Install answers now live only in `_bmad/config.toml` and
   * `_bmad/config.user.toml`; a leftover yaml is a stale second source of truth
   * that no shipped skill reads. Only files carrying the installer's own banner
   * are removed, so a hand-written or module-shipped config.yaml is left alone.
   * @param {string} bmadDir - BMAD installation directory
   */
  async _removeLegacyModuleConfigs(bmadDir) {
    let entries;
    try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (content.includes('# Generated by BMAD installer')) {
          await fs.remove(configPath);
        }
      } catch {
        // Unreadable — leave it in place rather than guess at its provenance
      }
    }
  }
  ...
  /**
   * Read the `[core] output_folder` answer from the central TOML config.
   * Walks the same layers skills resolve through (config_utils.load_central_config):
   * config.toml → config.user.toml → custom/config.toml → custom/config.user.toml,
   * with later layers winning, so a pinned override is honoured here too.
   * @param {string} bmadDir - BMAD installation directory
   * @returns {Promise<string>} Output folder path relative to the project root
   */
  async _readOutputFolder(bmadDir) {
    ...
    for (const layerPath of layers) {
      ...
      try {
        const value = parseCentralToml(await fs.readFile(layerPath, 'utf8')).core?.output_folder;
        if (typeof value === 'string' && value.trim() !== '') {
          outputFolder = value.trim();
        }
      } catch {
        // Unreadable layer — fall through to the next one
      }
    }

    // Default fallback
    if (!outputFolder) return '_bmad-output';

    // Strip {project-root}/ prefix if present
    return outputFolder.replace(/^\{project-root\}[/\\]/, '');
  }

  /**
   * Read the installer-owned `[core]` answers back out of the central TOML, so
   * an update that collected no core answers reuses the previous ones. Only the
   * two installer-written layers are read: `_bmad/custom/*.toml` holds pins that
   * must stay pins rather than being promoted back into install answers.
   * @param {string} bmadDir - BMAD installation directory
   * @returns {Promise<Object>} Merged [core] table, empty when nothing is readable
   */
  async _readInstalledCoreConfig(bmadDir) {
    const merged = {};
    for (const fileName of ['config.toml', 'config.user.toml']) {
      const layerPath = path.join(bmadDir, fileName);
      if (!(await fs.pathExists(layerPath))) continue;
      try {
        Object.assign(merged, parseCentralToml(await fs.readFile(layerPath, 'utf8')).core || {});
      } catch (error) {

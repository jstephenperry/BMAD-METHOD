---
title: 'Web Bundles'
description: Continuous Agile skills packaged for Google Gemini Gems and ChatGPT Custom GPTs
---

Run the planning side of Continuous Agile in your web LLM subscription, then bring the artifacts into your IDE.

## What is a Web Bundle?

A web bundle is a Continuous Agile skill repackaged for installation as a **Google Gemini Gem** or **ChatGPT Custom GPT**. Each bundle includes a `SKILL.md` protocol you upload as a knowledge file, an `INSTRUCTIONS.md` block you paste into the Gem or GPT instructions, and any data files the skill needs (CSVs, templates, validation checklists, additionally progressively disclosed content). The persona lives in the pasted instructions; the protocol lives in the knowledge file. Swap personas without touching the protocol.

Setup is not one-click, but the steps are guided. **Install from the bundle shelf published by BMad Code, LLC at [bmadcode.com/web-bundles](https://bmadcode.com/web-bundles/)**. The site lists every bundle in a card grid, shows you the Gemini and ChatGPT install steps inline, and hands you the ZIP download. That is the supported install path; the pattern is the same across the shelf, so once you've installed one the next one is mechanical.

Web bundles first shipped in V4. V6 brings them back, rewritten for the current Gem and Custom GPT platforms with Canvas, Deep Research, and image generation in mind.
...
| Product Brief Coach | Analysis | Mary (the analyst agent) |
...
Each bundle carries a default persona inherited from its owning agent (where one exists) and a contrasting swap example to demonstrate the voice change pattern.
...
4. **Hand off.** When you're done, you have a Canvas document you can export, paste into your repo, or feed to a Continuous Agile skill in your IDE for the next phase.
...
Web bundles are generated from skills using the `bmad-os-skill-to-bundle` utility skill. Point it at any skill folder and it produces the bundle files with persona inheritance from the owning agent.

Install any bundle from the BMad Code, LLC shelf at [bmadcode.com/web-bundles](https://bmadcode.com/web-bundles/).

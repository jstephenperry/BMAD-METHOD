...
Judge whether the plan can actually be built. The verdict is the deliverable — report it and stop.

Inventory what planning actually exists: scan `{planning_artifacts}` and `{project_knowledge}` (and the scope the user named, if any) for intent and planning artifacts — briefs, PRFAQs, PRDs, specs, UX outputs, architecture, epics and stories. Identify documents by reading what they are, not by filename patterns; projects arrive with different artifact mixes and naming.
...
- **PASS** — state it in one line and name what comes next: `bmad-build` on the first story
- **CONCERNS** — list them briefly with where each gap lives; ask `{user_name}` whether to proceed anyway or fix first
- **FAIL** — the plan is not implementable as recorded. Present findings ordered by severity, name the skill that fixes each (the relevant plan skill, or `bmad-correct-course` for cross-cutting changes), offer to save the findings to `{workflow.report_path}`, and stop

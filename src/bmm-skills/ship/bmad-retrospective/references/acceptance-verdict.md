...
When prior retros exist, check whether the action items they committed to were completed. Read the Action items section of each earlier `RETROSPECTIVE.md` in the sibling spec folders and, for every item not already recorded there as done, record one line in this retrospective document's Previous-retro follow-through section:

- **How to address the item** — the spec folder of the retrospective it came from plus its exact action text, character for character, so a reader can find it again.
...
That record is what Phase 5 carries into the document: the evidence is what the user is shown, and the proposed status is a proposal until the user closes the item in the earlier retrospective. A run with no prior retro, or one whose earlier retrospectives are unreadable or carry no action items, records that there was nothing to follow through on — and which of those it was, so a missing document is never mistaken for "no outstanding items."
...
`pending_stories` is authoritative for this epic's incomplete work: `stories.yaml` ids in list order whose artifact status is not `done`. When that list is non-empty:
...
If the completeness check did not run (no readable `stories.yaml`), do **not** render a rejected or accepted verdict from the absence of data — say the check was unavailable and weigh only the criteria and findings you have.
